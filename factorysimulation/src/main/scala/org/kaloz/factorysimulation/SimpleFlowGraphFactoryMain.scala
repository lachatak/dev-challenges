...
object SimpleFlowGraphFactoryMain extends App {
  ...
  val g = FlowGraph.closed() { implicit builder: FlowGraph.Builder[Unit] =>
    ...
    painting.out(1) ~> carFlow.map(_.copy(color = "green".some)) ~> merge
...
case class Wheel(serialNumber: String) {
  def this() = this(UUID.randomUUID().toString)
}

case class Coachwork(serialNumber: String) {
  def this() = this(UUID.randomUUID().toString)
}

case class Engine(serialNumber: String = UUID.randomUUID().toString)

case class Car(wheels: Seq[Wheel], coachwork: Coachwork, engine: Engine, color: Option[String] = None, serialNumber: String = UUID.randomUUID().toString)
